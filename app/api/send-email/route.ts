...
export async function POST(request: Request) {
  try {
    const { email, customerName, apartmentTitle, checkIn, checkOut, totalPrice, phone, adults, children } = await request.json()
    ...
    const data = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #f0f0f0; border-radius: 12px; padding: 24px;">
          ...
          <p>Avşa Cennet üzerinden yapmış olduğunuz konaklama talebi başarıyla tarafımıza ulaşmıştır. En kısa sürede inceleyip sizinle iletişime geçeceğiz.</p>
          ...
          <div style="background-color: #fff7ed; border-left: 4px solid #f97316; padding: 16px; margin: 20px 0; border-radius: 4px;">
            ...
            <p style="margin: 4px 0;"><strong>Seçilen Daire:</strong> ${apartmentTitle}</p>
            ...
            ${totalPrice ? `<p style="margin: 4px 0;"><strong>Tahmini Tutar:</strong> ₺${totalPrice}</p>` : ""}
          </div>
